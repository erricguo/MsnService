...
namespace MsnService
{
    public partial class Form1 : Form
    {
        ...
        public Form1()
        {
            ...
            txbServerport.Text = "2953";
            btnStop.Enabled = false;
        }
        
        private delegate void myUICallBack4(IPEndPoint ip);
        private void CheckUserList(IPEndPoint ip)
        {
            if (this.InvokeRequired) 
            {
                myUICallBack4 myUpdate = new myUICallBack4(CheckUserList);
                this.Invoke(myUpdate, ip);
            }
            else
            {
                for (int i = 0; i < userList.Count; i++)
                {
                    if (userList[i].GetIPEndPoint().Address.ToString() == ip.Address.ToString())
                    {
        ...
        private delegate void myUICallBack2(int i);
        private void DelUserToList(int i)
        {
            if (this.InvokeRequired)
            {
                myUICallBack2 myUpdate = new myUICallBack2(DelUserToList);
                this.Invoke(myUpdate, i);
            }
            else
            {
                lbUser.Items.RemoveAt(i);
        ...
        private delegate void myUICallBack3(int i,string name);
        private void ChangeUserNameToList(int i, string name)
        {
            if (this.InvokeRequired)
            {
                myUICallBack3 myUpdate = new myUICallBack3(ChangeUserNameToList);
                this.Invoke(myUpdate, i, name);
            }
            else
            {
                lbUser.Items[i] = name;
        ...
        private void ReceiveMessage()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    AddItemToListBox(splitstring[0] + message);
                    /*
                    switch (splitstring[0])
                    {
                        ...
                        case "login":
                            ...
                            CheckUserList(clientIPEndPoint);
                            // 往在线的用户列表添加新成员
                            userList.Add(user);
                            addUserToList(splitstring[1]);
                        ...
                        case "ChangeName":
                            for (int i = 0; i < userList.Count; i++)
                            {
                                if (userList[i].GetName() == splitstring[1])
                                {
                                    AddItemToListBox(string.Format("用户{0}({1})變更名稱", userList[i].GetName(), userList[i].GetIPEndPoint()));
                                    userList[i].SetName(splitstring[3]); // 移除用户
                                    ChangeUserNameToList(i, splitstring[3]);
                                }
                            }
                            for (int i = 0; i < userList.Count; i++)
                            {
            ...
            //AddItemToListBox(string.Format("服務線程{0}停止", serverIPEndPoint));
        }
        ...
        private void btnStop_Click(object sender, EventArgs e)
        {
            ...
            {
                // 广播注销消息
                SendtoClient(userList[i], "kickall,伺服器關閉");
            }

            while (userList.Count > 0)
            {
                AddItemToListBox(string.Format("用户{0}({1})退出", userList[0].GetName(), userList[0].GetIPEndPoint()));
                userList.RemoveAt(0); // 移除用户
                DelUserToList(0);
            }
            AddItemToListBox(string.Format("廣播:[{0}]", "踢掉全部用戶"));
            tcpListener.Stop();
        ...
        private void btnKick_Click(object sender, EventArgs e)
        {
            ...
            {
                // 广播注销消息
                //SendtoClient(userList[lbUser.SelectedIndex], "kick,"+userList[lbUser.SelectedIndex].GetName());
                SendtoClient(userList[i], "kick," + userList[lbUser.SelectedIndex].GetName());
            }

            for (int i = 0; i < userList.Count; i++)
            {
                if (userList[i].GetName() == lbUser.SelectedItem.ToString())
                {
                    name = lbUser.SelectedItem.ToString();
                    AddItemToListBox(string.Format("用户{0}({1})退出", userList[i].GetName(), userList[i].GetIPEndPoint()));
                    userList.RemoveAt(i); // 移除用户
                    DelUserToList(i);
                }
            ...
            AddItemToListBox(string.Format("廣播:[{0}]", "踢掉用戶" + name));
        }
        ...
        public static bool WriteLog(string xinfo, bool xWriteTime)
        {
            try
            {
                ...
                string filename = path + @"\" + DateTime.Now.ToString("yyyyMMdd") +"_Server"+ @".txt";
